...
CONFIG = {
    ...
    "public_engine_url": os.getenv("PUBLIC_ENGINE_URL", "https://animethic-direct-download.hf.space").rstrip("/"),
    "public_blogger_url": os.getenv("PUBLIC_BLOGGER_URL", "https://dl.animethic.xyz").rstrip("/"),
    ...
    "max_codes": int(os.getenv("MAX_CODES_PER_LINK", "2000")),
    "max_range": int(os.getenv("MAX_RANGE_SIZE", "2000")),
    ...
    "auto_index_uploads": env_bool("AUTO_INDEX_UPLOADS", False),
    "log_levels": {value.strip().lower() for value in os.getenv("LOG_LEVELS", "warn,error,store").split(",") if value.strip()},
    "log_min_interval": float(os.getenv("LOG_MIN_INTERVAL_SECONDS", "3")),
    "log_error_cooldown": int(os.getenv("LOG_ERROR_COOLDOWN_SECONDS", "300")),
    "poll_timeout": int(os.getenv("POLL_TIMEOUT_SECONDS", "20")),
    "poll_request_timeout": int(os.getenv("POLL_REQUEST_TIMEOUT_SECONDS", "30")),
    "cors_origin": os.getenv("CORS_ORIGIN", "*"),
    "max_users_per_bot": int(os.getenv("MAX_USERS_PER_BOT", os.getenv("MAX_STREAMS_PER_BOT", "25"))),
}
...
DIRECT_TOKENS = unique_tokens(csv_env("DIRECT_BOT_TOKENS", "DOWNLOAD_BOT_TOKENS", "BOT_TOKENS", "BOT_TOKEN"))
DIRECT_TOKEN_SET = set(DIRECT_TOKENS)
MAINTENANCE_TOKENS = [token for token in unique_tokens(csv_env("MAINTENANCE_BOT_TOKENS", "ADMIN_BOT_TOKENS")) if token not in DIRECT_TOKEN_SET]
...
LOG_LAST_SENT_AT = 0.0
LOG_LAST_BY_KEY = {}
LOG_SUPPRESSED_BY_KEY = {}
...
class IndexStore:
    ...
    def create_page_token(self, expression, ttl_seconds):
        return encode_page_token(expression, ttl_seconds)

    def get_page_expression(self, token):
        expression = decode_page_token(token)
        if expression:
            return expression
        now = int(time.time())
        with self.lock:
            ...
            expires_at = int(item.get("expires_at") or 0) if item else 0
            if not item or (expires_at and expires_at < now):
                return ""
            return item.get("expression", "")

    def create_download_token(self, code, ttl_seconds):
        return encode_download_token(code, ttl_seconds)

    def get_download_code(self, token):
        code = decode_download_token(token)
        if code:
            return code
        now = int(time.time())
        with self.lock:
    ...
    def cleanup_tokens(self, now=None):
        ...
        for key in ("links", "downloads"):
            ...
            for token in list(values.keys()):
                expires_at = int(values[token].get("expires_at") or 0)
    ...
    def stats(self):
        with self.lock:
            ...
            return {
                ...
                "autoIndexUploads": CONFIG["auto_index_uploads"],
                "logLevels": sorted(CONFIG["log_levels"]),
                "logMinIntervalSeconds": CONFIG["log_min_interval"],
                "logErrorCooldownSeconds": CONFIG["log_error_cooldown"],
            }
...
def home():
    return jsonify({
        ...
        "message": "Animethic Direct Downloads engine is running. Use /health or /api/create-link.",
        "stats": INDEX.stats(),
    })
...
def api_create_link():
    ...
    expression = request.args.get("start") or request.args.get("codes") or ""
    parsed = parse_post_codes(expression)
    ttl = int(request.args.get("ttl") or 0)
...
def build_blogger_link(expression, ttl):
    ...
    base_url = CONFIG["public_blogger_url"]
...
def encode_page_token(expression, ttl_seconds=0):
    return encode_secret_token("page", {"expression": expression}, ttl_seconds)


def decode_page_token(token):
    payload = decode_secret_token("page", token)
    expression = str(payload.get("expression") or "")
    if not expression:
        return ""
    try:
        parse_post_codes(expression)
        return expression
    except Exception:
        return ""


def encode_download_token(code, ttl_seconds=0):
    return encode_secret_token("download", {"code": str(code)}, ttl_seconds)


def decode_download_token(token):
    payload = decode_secret_token("download", token)
    code = str(payload.get("code") or "")
    return code if re.match(r"^\d+$", code) else ""
...
def decode_secret_token(kind, token):
    try:
        token = str(token or "")
        if not token.startswith("atx1."):
            return {}
        parts = token.split(".", 2)
        if len(parts) != 3:
            return {}
        _version, body, sig = parts
        expected = secret_token_signature(kind, body)
        if not hmac.compare_digest(sig, expected):
            return {}
        encrypted = base64.urlsafe_b64decode(pad_base64(body))
        raw = xor_bytes(encrypted, secret_token_stream(kind, len(encrypted)))
...
def public_file(item):
    code = item["code"]
    download_token = INDEX.create_download_token(code, 0)
    return {
...
def least_loaded_slot(available):
    ...
    with BOT_LOAD_LOCK:
        for slot, _ in available:
            ...
            if count < CONFIG["max_users_per_bot"]:
                return slot["slot"]
        return min(available, key=lambda pair: len(BOT_LOAD.get(pair[0]["slot"], [])))[0]["slot"]
...
def telegram_api(slot, method, payload, timeout=15):
    response = http_session().post(f"{CONFIG['bot_api_base_url']}/bot{slot['token']}/{method}", json=payload, timeout=timeout)
    try:
        data = response.json()
    except ValueError as exc:
        raise RuntimeError(f"Telegram {method} returned invalid JSON with HTTP {response.status_code}") from exc
...
def process_storage_update(slot, update):
    if not CONFIG["auto_index_uploads"]:
        return
...
def process_admin_update(slot, update):
    ...
    if command in {"/start", "/help"}:
        ...
        handle_admin_store(slot, chat_id, arg)
    else:
...
def handle_admin_link(slot, chat_id, expression):
    try:
        parsed = parse_post_codes(expression)
        send_admin_message(slot, chat_id, build_blogger_link(parsed["expression"], 0))
    except Exception as exc:
        send_admin_message(slot, chat_id, f"Could not create link: {exc}")


def handle_admin_store(slot, chat_id, raw):
    try:
        expression, title = parse_store_request(raw)
        parsed = parse_post_codes(expression)
    except Exception as exc:
        send_admin_message(slot, chat_id, f"Store usage: /store 1234-1240 = Title\nError: {exc}")
        return

    send_admin_message(slot, chat_id, f"Store started: {title}\nCodes: {len(parsed['codes'])}\nPlease wait while I check Telegram storage.")
    imported = []
    missing = []
    errors = {}
    ...
    for code in parsed["codes"]:
        ...
        if item:
            ...
            missing.append(code)
        if code_errors:
            errors[code] = code_errors[:3]

    link = build_blogger_link(parsed["expression"], 0)
    elapsed = max(1, int(time.time() - started_at))
    stored_names = [item.get("file_name", "") for item in imported[:8]]
    log_event("store", "Stored Animethic collection", {
        "title": title,
        "codes": parsed["expression"],
    ...
    lines = [
        f"Store complete: {title}",
        f"Imported: {len(imported)}/{len(parsed['codes'])}",
    ...
    if stored_names:
        lines += ["", "First files:"] + stored_names
    if missing:
        lines += ["", "Missing codes:", ", ".join(missing[:80])]
    if errors and missing:
        first_code = next(iter(errors))
        first_reason = errors[first_code][0].get("reason", "Unknown error")
        lines += ["", f"First error: {first_code} - {first_reason}"]
    send_admin_message(slot, chat_id, "\n".join(lines))


def parse_store_request(raw):
    raw = (raw or "").strip()
    if not raw:
        raise ValueError("Missing codes and title.")
    if "=" in raw:
        expression, title = raw.split("=", 1)
    else:
        parts = raw.split(None, 1)
        expression = parts[0]
        title = parts[1] if len(parts) > 1 else parts[0]
    expression = expression.strip()
    title = re.sub(r"\s+", " ", title.strip()) or expression
    if len(title) > 180:
...
def admin_help():
    return "\n".join([
        ...
        "/store 1234-1240 = Title - index and store a collection",
    ])
...
def start_pollers():
    if not CONFIG["enable_polling"] or CONFIG["enable_webhooks"]:
        return
    if CONFIG["auto_index_uploads"]:
        for slot in DIRECT_SLOTS:
            threading.Thread(target=poll_storage_forever, args=(slot,), daemon=True).start()
    elif not MAINTENANCE_SLOTS:
        for slot in DIRECT_SLOTS:
...
def prepare_polling(slot):
    try:
        telegram_api(slot, "deleteWebhook", {"drop_pending_updates": False}, timeout=8)
    except Exception as exc:
        log_event("warn", f"{slot['name']} could not clear webhook before polling", {"reason": str(exc)})


def poll_storage_forever(slot):
    prepare_polling(slot)
    offset_key = f"storage-{slot['slot']}"
    offset = int(read_offsets().get(offset_key, 0))
    while True:
        try:
            updates = telegram_api(slot, "getUpdates", {"offset": offset, "timeout": CONFIG["poll_timeout"], "allowed_updates": ["message", "channel_post"]}, timeout=CONFIG["poll_request_timeout"])
...
def poll_admin_forever(slot):
    ...
    while True:
        try:
            updates = telegram_api(slot, "getUpdates", {"offset": offset, "timeout": CONFIG["poll_timeout"], "allowed_updates": ["message"]}, timeout=CONFIG["poll_request_timeout"])
            for update in updates:
                offset = int(update["update_id"]) + 1
                write_offset(offset_key, offset)
                process_admin_update(slot, update)
...
def log_event(level, text, details=None):
    ...
    if CONFIG["log_levels"] and level not in CONFIG["log_levels"]:
        return
    details = details or {}
    reason = str(details.get("reason") or "")
    key = "|".join([level, text, reason[:180]])
    suppressed = 0
    global LOG_LAST_SENT_AT
    with LOG_LOCK:
        now = time.time()
        cooldown = CONFIG["log_error_cooldown"] if level in {"warn", "error"} else 0
        last_same = LOG_LAST_BY_KEY.get(key, 0)
        if cooldown and now - last_same < cooldown:
            LOG_SUPPRESSED_BY_KEY[key] = LOG_SUPPRESSED_BY_KEY.get(key, 0) + 1
            return
        suppressed = LOG_SUPPRESSED_BY_KEY.pop(key, 0)
        wait_for = CONFIG["log_min_interval"] - (now - LOG_LAST_SENT_AT)
        if wait_for > 0:
            time.sleep(wait_for)
        LOG_LAST_SENT_AT = time.time()
        LOG_LAST_BY_KEY[key] = LOG_LAST_SENT_AT
    if suppressed:
        details["suppressed_similar"] = suppressed
